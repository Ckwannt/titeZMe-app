...
export function ShopPhotosTab({ shop, mutateShop }: { shop: any, mutateShop: () => void }) {
  const { user } = useAuth();
  ...
  const handlePhotoUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (photos.length >= 30) {
      setErrorMsg('Maximum 30 photos allowed.');
      return;
    }
      ...
      <section className="mb-12">
        ...
        {photos.length === 0 ? (
          <div className="border border-dashed border-[#333] rounded-3xl p-8 text-center text-[#888] bg-[#0a0a0a]">
            No shop photos yet. Add some to show off your space!
          </div>
        ) : (
            ...
            {videos.map((url: string, i: number) => (
              <div key={i} className="aspect-[4/5] bg-[#141414] rounded-2xl overflow-hidden relative group border border-[#2a2a2a]">
                ...
                <button 
                  ...
                  className="absolute top-2 right-2 w-8 h-8 rounded-full bg-black/60 text-white flex z-10 items-center justify-center opacity-0 group-hover:opacity-100 backdrop-blur-sm transition-all hover:bg-brand-red pointer-events-auto"
                >
                  ✕
                </button>
              </div>

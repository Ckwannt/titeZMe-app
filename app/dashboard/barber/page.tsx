...
export default function BarberDashboard() {
  ...
  // Sync service currency with barber profile
  useEffect(() => {
    if (profile?.currency) setServiceCurrency(profile.currency);
  }, [profile?.currency]);
  ...
  const addService = async () => {
    ...
    // Improvement 6 — Duplicate prevention
    const isDuplicate = services.some(s =>
      s.name.toLowerCase().trim() === newServiceName.toLowerCase().trim()
    );
    if (isDuplicate) {
      setToastMessage(`You already have a service called '${newServiceName}'. Edit the existing one instead.`);
  ...
  // Improvement 2 — Inline edit save
  const saveEditService = async (svcId: string) => {
    try {
      await updateDoc(doc(db, 'services', svcId), {
        name: editSvcData.name,
        duration: parseInt(editSvcData.duration) || 30,
        price: parseFloat(editSvcData.price) || 0,
  ...
  // Improvement 5 — Active/inactive toggle
  ...
  // Improvement 4 — Reorder with up/down
  const moveService = async (svcId: string, direction: 'up' | 'down') => {
    const sorted = [...services].sort((a: any, b: any) => (a.sortOrder ?? 999) - (b.sortOrder ?? 999));
  ...
  // Improvement 1 — Update currency
  const updateServiceCurrency = async (newCurrency: string) => {
    if (!user) return;
    setServiceCurrency(newCurrency);
    try {
      await updateDoc(doc(db, 'barberProfiles', user.uid), { currency: newCurrency });
      mutateProfile();
  ...
  const sortedServices = [...services].sort((a: any, b: any) => (a.sortOrder ?? 999) - (b.sortOrder ?? 999));
  const svcActiveList = services.filter((s: any) => s.isActive !== false);
        ...
        {activeTab === "Invites" ? (
          ...
          <div className="animate-fadeUp max-w-[600px]">
              ...
              <div className="flex flex-col gap-3 mb-8">
                {/* Improvement 7 — Summary */}
                <div className="text-[11px] text-[#555] font-bold mb-1">
                  {svcActiveList.length === 0
                    ? 'No services added yet'
                    : `${svcActiveList.length} active service${svcActiveList.length !== 1 ? 's' : ''} · from ${svcSym}${svcMin} to ${svcSym}${svcMax}`}
                </div>

                {/* titeZMe Cut — unchanged */}
                ...
                {/* Sorted services — with all improvements */}
                {sortedServices.map((svc: any, idx: number) => (
                  editingServiceId === svc.id ? (
                    /* Improvement 2 — Inline edit mode */
                    <div key={svc.id} className="bg-brand-surface border border-brand-border rounded-2xl p-4">
                      <div className="grid grid-cols-[2fr_1fr_1fr] gap-2 mb-2">
                        <input value={editSvcData.name} onChange={e => setEditSvcData(p => ({...p, name: e.target.value}))}
                          className="bg-[#141414] border border-[#2a2a2a] rounded-xl px-3 py-2 text-white text-sm outline-none focus:border-brand-yellow" placeholder="Service name" />
                        <input value={editSvcData.duration} onChange={e => setEditSvcData(p => ({...p, duration: e.target.value}))}
                          className="bg-[#141414] border border-[#2a2a2a] rounded-xl px-3 py-2 text-white text-sm outline-none focus:border-brand-yellow" type="number" placeholder="Mins" />
                        <input value={editSvcData.price} onChange={e => setEditSvcData(p => ({...p, price: e.target.value}))}
                          className="bg-[#141414] border border-[#2a2a2a] rounded-xl px-3 py-2 text-white text-sm outline-none focus:border-brand-yellow" type="number" placeholder="Price" />
                      </div>
                      <textarea value={editSvcData.description} onChange={e => setEditSvcData(p => ({...p, description: e.target.value}))}
                        maxLength={150} rows={2} placeholder="Description (optional)"
                        className="w-full bg-[#141414] border border-[#2a2a2a] rounded-xl px-3 py-2 text-white text-sm outline-none focus:border-brand-yellow resize-none mb-3" />
                    ...
                    <div key={svc.id} className={`bg-brand-surface border border-brand-border rounded-2xl p-4 flex items-center gap-3 ${svc.isActive === false ? 'opacity-50' : ''}`}>
                      {/* Improvement 4 — Up/down arrows */}
                      <div className="flex flex-col gap-0.5 shrink-0">
                        <button onClick={() => moveService(svc.id, 'up')} disabled={idx === 0}
                          className="text-[#444] hover:text-[#888] text-xs leading-none disabled:opacity-20">↑</button>
                        <button onClick={() => moveService(svc.id, 'down')} disabled={idx === sortedServices.length - 1}
                          className="text-[#444] hover:text-[#888] text-xs leading-none disabled:opacity-20">↓</button>
                      </div>
                      {/* Improvement 5 — Active toggle */}
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {/* Improvement 3 — Description display */}
                      ...
                      <div className="flex items-center gap-2 shrink-0">
                        ...
                        {/* Improvement 2 — Edit button */}
                        <button onClick={() => { setEditingServiceId(svc.id); setEditSvcData({ name: svc.name, duration: String(svc.duration || ''), price: String(svc.price || ''), description: svc.description || '' }); }}
                          className="text-[#444] hover:text-[#888] font-bold text-xs p-1.5">✏️</button>
                        <button onClick={async () => { await deleteDoc(doc(db, 'services', svc.id)); mutateServices(); }}
                          className="text-[#555] hover:text-brand-red font-bold text-xs p-1.5">✕</button>
                      </div>
                    </div>
                  )
                ))}
              ...
              <div id="add-service-form">
                ...
                <div className="grid grid-cols-[2fr_1fr_1fr] gap-2.5 items-end mb-2">
                  ...
                  <div>
                    <label className="text-[10px] font-extrabold text-brand-text-secondary block mb-1.5">PRICE {svcSym}</label>
                ...
                {/* Improvement 3 — Description textarea */}
                <div className="mb-2">
                  <label className="text-[10px] font-extrabold text-brand-text-secondary block mb-1.5">DESCRIPTION (optional)</label>
                  <textarea value={newServiceDescription} onChange={e => setNewServiceDescription(e.target.value)}
                    maxLength={150} rows={2} placeholder="What does this service include?"
                    className="w-full bg-[#141414] border-[1.5px] border-[#2a2a2a] rounded-xl px-4 py-2 text-white text-sm outline-none transition-colors focus:border-brand-yellow resize-none" />
                  <div className="text-[10px] text-[#444] text-right mt-0.5">{newServiceDescription.length}/150</div>
                </div>
                {/* Improvement 1 — Currency selector */}
                <div className="flex items-center gap-2 mb-3">
                  <label className="text-[10px] font-extrabold text-brand-text-secondary uppercase">Currency:</label>
                  <select value={serviceCurrency} onChange={e => updateServiceCurrency(e.target.value)}
                    className="bg-[#141414] border border-[#2a2a2a] text-white text-xs rounded-lg px-3 py-1.5 outline-none focus:border-brand-yellow transition-colors">
                ...
                {/* Improvement 8 — Quick-add chips */}
                <div className="mt-4">
                  <div className="text-[10px] text-[#555] font-bold mb-2">Quick add:</div>
                  <div className="flex flex-wrap gap-2">
                    {QUICK_CHIPS.map(chip => {
                      const exists = services.some((s: any) => s.name.toLowerCase() === chip.name.toLowerCase());
                      return (
                        <button key={chip.name}
                          disabled={exists}
                          onClick={() => {
                            setNewServiceName(chip.name);
                            setNewServiceDuration(String(chip.duration));
                            setNewServicePrice('');
                            document.getElementById('add-service-form')?.scrollIntoView({ behavior: 'smooth' });
                          }}
                          className={`inline-flex items-center gap-1 text-[11px] font-bold rounded-full px-3 py-1 border transition-colors ${
                            exists
                              ? 'border-[#1e1e1e] text-[#444] bg-[#111] cursor-not-allowed'
                              : 'bg-[#141414] border-dashed border-[#2a2a2a] text-[#666] hover:border-brand-yellow hover:text-brand-yellow cursor-pointer'
                          }`}>
                          {exists ? '✓' : '+'} {chip.name}
                        </button>
                      );
